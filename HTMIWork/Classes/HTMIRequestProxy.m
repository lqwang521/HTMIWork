...
{
//如果有加密需求，可以在这个类里设置公共的一些加密字段及value
...
//wlq add
- (NSInteger)callFormUrlWithParams:(NSDictionary *)params url:(NSString *)url headers:(NSDictionary *)headers methodName:(NSString *)methodName success:(HTMIProxyCallback)success fail:(HTMIProxyCallback)fail
{
    //    headers = [NSDictionary sjHeaderForPOSTComplementWithHeader:headers];
    //这个比较特殊，需要放到请求中具体设置
    /*
...
- (NSNumber *)callApiWithParams:(NSDictionary *)params url:(NSString *)url methodName:(NSString *)methodName requestType:(RequestType)type success:(HTMIProxyCallback)success fail:(HTMIProxyCallback)fail upload:(multipart)upload
{
    ...
    switch (type) {
        ...
        case REQUEST_FormUrl:{
            ...
            NSString *accessToken =  [self defaultLoadAccessToken];
            NSString *refreshToken =  [self defaultLoadRefreshToken];
